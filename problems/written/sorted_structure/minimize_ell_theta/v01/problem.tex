\begin{progprob}
    ...
    \begin{subprobset}
        ...
        \begin{subprob}
            ...
            In a file named \python{min_ell_theta.py}, write an efficient function called
            ...
            the picture below. You may \emph{not} assume that \python{data} is
    ...
    \end{subprobset}
        ...
        Now suppose that a small number of the red points are less
    ...
    \begin{subprobset}
        ...
        \begin{subprob}
            Also in \python{min_ell_theta.py}, write a function named
        ...
        \begin{subprob}
            Also in \python{min_ell_theta.py}, write a function named \python{minimize_ell(data, colors)}
        ...
        \begin{subprob}
            Now assume that \python{data} is sorted (and \python{colors[i]}
            is the color of \python{data[i]}). In the file called
            \python{min_ell_theta.py}, write a function
            \python{minimize_ell_sorted(data, colors)} which returns a minimizer $\theta$
            in linear time. Your code should satisfy the loop invariant: ``After the
            $\alpha$th iteration, \python{blue_gt_theta} is the number of blue points
            which are greater than \python{data[}$\alpha - 1$\python{]}.''

            For simplicity, suppose that exactly $n/2$ of the data points are
            \python{'red'}, and $n/2$ are \python{'blue'}.

            \begin{soln}
                We can use the knowledge that exactly half the data points are
                red and the other half are blue to track the amount of red elements 
                and blue elements on both sides of $\theta$ on each iteration. Using
                the count of red elements before $\theta$ and blue elements after $\theta$
                we can calculate loss in constant time. This approach takes $\Theta(n)$ 
                time in total.
                
                \inputminted{python}{\thisdir/include-solution/minimize_ell_theta.py}
            \end{soln}
        \end{subprob}
